...
class ImageTranslateGUI:
    def __init__(self, root):
        ...
        backend_frame.grid(row=4, column=1, columnspan=2, sticky="w")
        ...
        tk.Checkbutton(main_frame, text="Use GPU if available", variable=self.use_gpu_var).grid(
            row=5, column=1, columnspan=2, sticky="w", padx=10, pady=4
        )

        tk.Label(main_frame, text="Program Output:").grid(row=6, column=0, padx=10, pady=10, sticky="w")
        self.output_text = tk.Text(main_frame, height=10, width=66, wrap=tk.WORD)
        self.output_text.grid(row=7, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")
        ...
        self.translate_button.grid(row=8, column=1, pady=20)
